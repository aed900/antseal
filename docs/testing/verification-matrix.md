...
| V1.2 | 167 | Committed golden vectors | M0 | Q/F/C/G/R | covered | `crates/antseal-core/tests/vector_runner.rs::vector_runner_discovers_and_executes_every_committed_vector`, `crates/antseal-core/tests/vector_index.rs::vector_index_exists_and_matches_the_committed_tree`, `testdata/vectors/v1` | Runner discovers by directory walk, no hardcoded list. Seven vector families under `v1/`. |
...
| V3.4 | 169 | **bundle/manifest CBOR fuzzing in CI** | M0 | F17 + Q9 | **gap** | NONE — see notes | **This is the one M0 bullet of lines 165–175 with no test behind it, and it blocks the Q14 gate.** `fuzz/fuzz_targets/` holds exactly one target, `verify_bundle.rs` (R10's), which drives `verify_bundle` — not the manifest or bundle CBOR decoders the spec names. The CI job `fuzz-smoke` is a reserved mount point whose only step prints "Green here asserts NOTHING until Q9 lands the smoke run." And it could not run if the targets existed: `rust-toolchain.toml` pins stable 1.92.0 while cargo-fuzz needs nightly, which `fuzz/Cargo.toml` records in its own header. So three things are missing, not one: the targets (F17), the nightly pin (**Q39**), and the lane (Q9). Partial mitigation that does run today, and is **not** a substitute: `crates/antseal-core/tests/verify_fuzz.rs::arbitrary_bytes_never_panic` and `crates/antseal-core/tests/codec_properties.rs::arbitrary_bytes_never_panic_the_decoder` are in-suite property tests over arbitrary bytes, sharing the mutation engine with the fuzz target — they exercise the same surface with vastly less input volume and no coverage feedback. |
