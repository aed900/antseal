...
**[2026-09-13, wave 35 — `R97`: that commit no longer exists, and the page has been redeployed from one that does.]** `9317a35` was rewritten by the wave-31 identifier removal and garbage-collected by the Support request that closed `Q268` — the API answers *422 No commit found* — so from that collection until today the live artifact reproduced from **no existing commit**. Redeployed under the maintainer's express consent given 2026-09-13 in direct reply to a request naming each action, the destination `github.com/aed900/antseal` and the account `aed900`: `workflow_dispatch` run `34782645736` at `bfdc93eba79883c79f79fe663cb4a24d024f0d03`, 12 steps, every one `success`, runner assigned. The verify step printed `served bytes match the built artifact: b441bae727015ef2fb217fb071f7c6ae3fb31cde92f0af0900d146e472e00b1d`; an independent fetch returned the same sha256 (2 542 496 B, `last-modified: Sun, 13 Sep 2026 21:08:21 GMT`); and the served module — 1 867 870 B, sha256 `edd62f059c25c48674f47274fb8cb713fb4a2f83e3bf3d3fcbd54241040da2f3` — contains `bfdc93e…` once, `9317a35…` zero times and a same-alphabet control zero times, with the footer now labelled `module build`, its digest equal to the module's, and one signing-key element. **That module digest is byte-identical to the digest the first hosted `reproducible-build` run reproduced in two environments at the same commit (run `34781450856`)**, so *reproducible at the commit it was deployed from* is, for the first time, witnessed on hosted runners from a commit that exists. **A history rewrite must now enumerate the live deploy commit as an input**, exactly as it must a SHA pin in `scripts/`.
