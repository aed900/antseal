...
14. `docs/decisions/README.md` — this decision's index row. **Applied
    2026-08-11** under
    [D119](D119-decision-index-identity-and-the-index-row-sections.md)
    RULING 4, which also deleted the `## Index row` section this line used to
    point at ("the index row at the foot of this file"); the row itself now
    lives only in the index, and RULING 6 puts it in the act that commits the
    record.
