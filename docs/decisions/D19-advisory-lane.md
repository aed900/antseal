...
## Amendment (2026-07-27, from the C11 probe — folded at wave-2 integration)
...
cargo-deny remains the lane's tool, but **Q10's weekly operation MUST
additionally sweep GHSA/osv.dev for the exact-pin-class crates** (the
watchlist above). Recommended mechanism: a scripted osv.dev API query over
the exact-pin list, run from the weekly `advisory-cron.yml` workflow —
standing Q10 follow-up, tracked there.
...
- Q10 owns permanent operation (new-advisory → tracking issue workflow +
  the GHSA/osv.dev sweep above), the licenses completion after Q29, and
  the level review of `multiple-versions` once the M1 graph is real.
