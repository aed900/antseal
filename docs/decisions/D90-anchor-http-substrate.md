...
```
| [D90](D90-anchor-http-substrate.md) | Anchor network substrate — `ureq =3.3.0` (`rustls`, no default features), **blocking**, **ungated**, declared by `antseal-anchor` alone: default graph **129 → 144**, ant-backend 447 → 451, devnet 475 → 479. No runtime created, borrowed or required — the frozen `AnchorGate::run` `async fn` completes in **one poll** under the existing noop-waker executor; independence is `std::thread::scope` (1.503 s wall for a 1.5 s-slow + fast pair). Gating refused on D89's ground: required CI is default-features-only, so every Accept row of A3/A10/A13/A16/A17 would be compiled by nothing. `reqwest` measured at **+64…+72** default packages vs ureq's **+15**, and its `blocking` feature is a background tokio runtime. Three ureq behaviours found only by probe: `.limit(N)` accepts **N−1** bytes, `max_redirects(0)` **returns** the 3xx, and `Config::default` reads `HTTP_PROXY` — which would collapse A16's must-agree pair onto one origin. HTTPS mandatory for esplora/RPC (unsigned payloads), plain HTTP safe for RFC 3161 (signed + nonce-bound) and unavoidable: `timestamp.digicert.com:443` refuses connections. A non-2xx failure **carries its body** — D58's three-way calendar discriminator is a 404 *body*, and the draft that discarded it would have made A14 unimplementable. Receive caps sized from measurement, not round numbers: calendar reply **64 KiB** (largest of 18 real replies 220 B; `MAX_OTS_BYTES` caps the *merged* artifact, a different quantity, and F4 makes a too-high cap permanent), Arbitrum RPC **512 KiB** (D37's 256 transfers = a 188 328 B receipt, so the draft's 64 KiB would have failed A17 on exactly the large seals it exists to corroborate). Agreement is over an extracted tuple, never bytes — the two ruled-in RPCs return different JSON key sets for identical values. Also found: the `core-dep-graph` forbidden-crate scan is the only rule in that lane with no self-test, and its denylist is measurably weaker than its headline (`libc`, `regex`, `env_logger` all pass) — adjacent to Q74, not blocked by it | RESOLVED (A3 implements) | 2026-08-02 |
```
