...
- Dev-tools whose output or verdict the project depends on are exact-pinned
  ...
  **Python `cbor2` `==6.1.3`** (the D12 independent CBOR cross-check the
  ...
  any Rust dependency tree), **cargo-fuzz `=0.13.2`** (its verdict gates
  merges via the `fuzz-smoke` lane; pinned at Q9 — installed with
  ...
  silently producing a verdict from an unreviewed tool). The M3
