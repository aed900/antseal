...
### F38 — A synthesized fixture's base has no provenance check; a committed one does
- Milestone: M0
- Size: S
...
- Discovered by: **F22** (2026-07-28). F15 established that a fixture base is not merely documented but **diffed**: `the_bases_are_the_committed_golden_vectors` asserts both committed bases are byte-identical to their F12/F13 vector cases every run, so "derived from the golden vectors" fails loudly the day it stops being true. F22's `bundle-cert-over-cap` needed a bundle that actually carries an anchor artifact — neither committed base does, both anchor sections being empty — so it uses R6's `AnchorSet::EveryKind` shape over the `single binary` ed25519-only work. That base is **not committed and not diffed against anything**: it is asserted only to decode and to carry an intermediate. The F13 golden vectors do commit an every-anchor-kind bundle, but over `multi_file` with the hybrid policy, so the two are not comparable byte-for-byte. The gap is small today and structural: it is the one fixture base in the repository whose provenance is a constructor call rather than a checked artifact, and the pattern will be copied the next time a fixture needs a shape the two bases lack.
...
- Accept:
  ...
  - Whatever is chosen applies to *any* future synthesized fixture's base, not just this one — the emitter records it uniformly.
  - No committed fixture grows past the 64 KiB ceiling, and no 256 MiB/16 MiB buffer becomes a file.
- Notes: The two size-cap recipes (`bundle-oversized`, `manifest-oversized`) do not have this problem: their bases *are* the two committed, diffed ones. It is specifically the third base that is unanchored.
