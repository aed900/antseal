...
- Notes: P20 extends this containment class to the devnet era (default `--workspace` lanes never compile `ant-node`; alloy↔evmlib lockstep). D35's revisit triggers stand: the P19 weekly check watches for an upstream relicense, and the v1.1 large-blob decision would reopen the dependency question.
