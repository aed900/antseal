...
> **Maintainer actions**: (1) **P2 — promptly**: `cargo login` then `scripts/reserve-crates.sh --execute` (all 5 names re-verified FREE + dry-run green 2026-07-27 ~18:58Z; availability decays); (2) **P3 — promptly**: register antseal.org (primary) + antseal.dev (defensive), auto-renew on (both re-verified FREE ~18:58Z; docs/naming/P3-domain-registration.md); (3) `gh auth refresh -h github.com -s workflow` then `git push origin main` → confirm CI lanes green (3-OS matrix; **now 14 required contexts** — `secret-guard` added at Q2; wave-2 payload in docs/ci-verification.md) → wasm-guard probe → THEN branch protection per runbook (P8+Q1); weekly advisory-cron activates on the same push (P13); (4) optional courtesy: file docs/naming/upstream-blessing-request.md at WithAutonomi/ant-client (nothing waits on it).
...
- [ ] **P13** (S) RUSTSEC advisory tracking lane (cargo-deny/audit, weekly schedule) ⏳ WIP — local complete 2026-07-27 (D19): deny.toml (advisories/bans/sources active; licenses stubbed → Q29) + `audit-deny` per-PR lane + weekly advisory-cron.yml, cargo-deny =0.19.8 pinned, local run green ⛔ remote lane evidence + red-lane demo blocked on the P8 push blocker
...
- [x] **D2** Repo hosting platform + CI provider (assumption: GitHub + Actions) (P — blocks P4, P8) — **Resolved 2026-07-27**: GitHub + GitHub Actions; private repo `aed900/antseal` (docs/decisions/D2-hosting-ci.md) — **amended same day: relocated to `aed900/antseal`** (identity rewrite to aed900 before first push; old aed900 repo deprecated, deletion pending maintainer action)
