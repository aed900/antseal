...
> **Phase: M0 — wave 2 executed 2026-07-27** (planning pass resolved D19/D20/D21 first → 6 parallel worktree agents + orchestrated integration → R2 follow-up agent): F2 ✅ F3 ✅ C5–C9 ✅ C11 ✅ (**ml-dsa wasm32 VIABLE — executed bit-match**) G2 ✅ Q2/Q3/Q4 ✅ R2 ✅ (G13-rider) · P11/P12 ✅ · P13 ⏳ (local complete; remote lane evidence blocked on P8) · F4 ⏳ (draft committed; freeze at Q14). D13/D14/D16/D19/D20/D21 resolved; D9/D17 proposals + D15 inputs captured; D30 at 71 stable codes (freeze at Q7/Q8). Full local gate green post-integration: fmt/clippy/test (**223 tests**)/wasm32/core-dep-graph/cargo-deny.
> **Next wave (unblocked now)**: F5→F6→F7 (manifest schema + envelope on the F4 draft; tighten D8 §13 items with A/S) · C10 (manifest AEAD) · C12→C13→C14 (signature impls on the D13/D14 pins; C12 pre-validation per D16; D15 decision-first at C13) · G3 (corpus — include BOM-only, double-BOM, truncated-BOM, invalid-UTF-8 maximal-subparts rows) · G4/G5 (descriptor + unit model; D22/D23/D24 decision-first) → G6/G7/G8 · P14 (wasm toolchain — C11's recorded getrandom lines feed the recipe audit) · Q5 (wasm bit-match harness; Q4's executor is already WASM-safe) · R3 (structural checks) · Q7 (tamper harness + D30 formalization; C14 ratifies per-alg codes).
...
- [ ] **D9** GGM node-address representation ((level,index) vs bit-path) — co-frozen G8/F4 — 2026-07-27 both forms drafted in the F4 registry with **(level,index) recommended** (the spec's MSB-first leaf rule generalized; canonical for free vs bit-path's pad-bit rule); co-freeze with G8
...
- [ ] **D15** ML-DSA signing mode: hedged vs deterministic (affects golden-vector shape) (C16) — 2026-07-27 inputs captured (C11): crate default = deterministic (rnd=0³²), hedged behind a rand_core feature; deterministic sigs are cross-implementation byte-identical (fips204 check) → portable C16 vectors favor deterministic; resolve at C13/C16
...
- [ ] **D17** Registered `sig_policy` algorithm-ID numeric values + reserved slots (C14/F4) — 2026-07-27 proposal drafted (F4 registry: **0 = ed25519, 1 = ml-dsa-65, 2–15 reserved**; ed25519 = 0 because the fallback policy is `[0]`); freeze with F4/C14 at Q14
...
- [ ] **D30** Stable machine-readable error-code contract across F/C/G/A/R (Q7/Q8) — 2026-07-27 progressing: kebab-case `code()` contract materialized across F (`cbor-*`, 15) + C (`crypto-*`, 25; per-kind/per-alg) + R wrapper arms — **71 distinct codes**, exemplar-enforced distinctness meta-tests; formal freeze remains Q7/Q8 (C14 to ratify per-alg signature codes)
