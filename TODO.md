...
- [ ] **D80** Must a revealed unit's owning file appear in `touched_files`? (F8/R4, D8 §13 item 10) — surfaced 2026-07-28. `full_reveals ⊆ touched_files` is already settled mandatory; this is the weaker per-unit case. **Genuine trade-off**: requiring it kills path-hiding selective disclosure (proving a paragraph existed without disclosing which file held it, since `touched_files` carries the path + `path_salt` that open `path_commit`); not requiring it is the privacy-preserving read. Note the asymmetry — **adding** the requirement later breaks old bundles, **dropping** it later does not, so tolerant-now is the *irreversible* direction here (the opposite of D28's). Decide with spec evidence at F8; if genuinely ambiguous, stop rather than freeze
