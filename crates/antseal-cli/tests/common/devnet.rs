...
pub fn devnet() -> Option<(NetworkConfig, DevnetEnv)> {
    ...
    // These suites PAY, so they need a funded key whatever the mode. Ten
    // keys is the local devnet and nothing changes; nine is
    // arbitrum-sepolia, whose key is real key material and is never written
    // to a file, so the documented recipe supplies it through the process
    // environment (docs/devnet/sepolia-devnet.md). If neither, the skip
    // names the key and the mode instead of calling the devnet absent.
    let env: DevnetEnv = if env.wallet_private_key().is_some() {
        ...
        eprintln!(
            "SKIP: the devnet export at {} carries no {} — the designed shape of an \
             ...
             material and is never written to a file. These suites pay, so export {} with a \
             key funded on that chain (docs/devnet/sepolia-devnet.md) to run them.",
            path.to_string_lossy(),
