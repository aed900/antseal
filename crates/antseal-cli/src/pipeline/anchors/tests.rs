...
fn the_reader_re_sorts_numerically_because_the_directory_order_is_lexical() {
    ...
    let anchors =
        StoredAnchors::assemble(twelve_captures_in_directory_order()).expect("well-formed slots");
...
fn a_work_with_no_anchor_slots_reads_as_empty() {
    let anchors = StoredAnchors::assemble(Vec::new()).expect("no slots is not a failure");
    assert!(anchors.is_empty());
...
fn the_reader_refuses_a_slot_set_it_cannot_read_wholly() {
    ...
    for alien in ["ots-upgrade", "tsa-01", "tsa-", "tsa-x", "notes.der", "0"] {
        ...
        match StoredAnchors::assemble(records) {
            Err(JournalError::Corrupt { detail }) => assert_eq!(
    ...
    match StoredAnchors::assemble(mismatched) {
    ...
    assert!(
        StoredAnchors::assemble(vec![(OTS_SLOT.to_owned(), upgraded_fixture())]).is_ok(),
