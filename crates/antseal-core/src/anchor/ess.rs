...
/// `all_nine_tsas_verify_or_fail_for_a_named_algorithm_reason` measures that
/// this holds on all nine live captures: binding the signer certificate is a
/// precondition of every one of them reaching a verdict, so a name-encoding
/// difference in any capture would surface there.
