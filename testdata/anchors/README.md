...
Four of the five record their maintainer authorisation verbatim in their own
...
| `A25-wave17-cycle/` | 2026-08-11 (day-1 submits 23:39Z through `scripts/anchor-smoke`) | 9 | **The first campaign whose submissions were issued by antseal's own client** — the residue `V7.1` has recorded since 2026-08-02, and the reason this campaign exists. Day-1: eight **pending** submissions, the two committed golden-vector digests at the four `DEFAULT_OTS_CALENDARS` pool endpoints, every one `pending-accepted` through A13's `submit_to_calendars` (every earlier campaign's pendings were hand-made with `curl` before the client existed). Confirms D127 §2's measurement live: the pool endpoints answer with the **calendar** hosts (`alice`/`bob`.btc.calendar.opentimestamps.org, `finney.calendar.eternitywall.com`, `btc.calendar.catallaxy.com`), which is why contacting the pool set was never implied by the upgrade legs that spoke to those calendar hosts. Consent recorded verbatim in the log header per runbook §1. **The upgrade half is owed** under its own confirmation — until one of these commitments is upgraded through A14, D127 §5's closure predicate is unmet and `V7.1` stays `gap`. |
